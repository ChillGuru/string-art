...
function NonBlockingCalculatePins(){
    ...
    (function codeBlock(){
        if(i < N_PINS){
            ...
            status.textContent = "Гвозди расположены";
            showStep(2);
...
function NonBlockingPrecalculateLines(){
    ...
    status.textContent = "Высчитываем расположение гвоздей...";
    ...
    (function codeBlock(){
        if(a < N_PINS){
            ...
            console.log('Траектория линий высчитана');
            status.textContent = "Траектория линиий высчитана";
...
function NonBlockingLineCalculator(){
    ...
    status.textContent = "Рисуем линии...";
...
function Finalize() {
    ...
    save_sequence = line_sequence.slice();
    for (var i=0; i < save_sequence.length; i++){
        if (Math.floor((save_sequence[i] -1) / 60) <= 0){
            save_sequence[i] = 'A' + save_sequence[i];
        }
        else if (Math.floor((save_sequence[i] -1) / 60) == 1){
            save_sequence[i] = 'B' + (save_sequence[i]-60);
        }
        else if (Math.floor((save_sequence[i] -1) / 60) == 2){
            save_sequence[i] = 'C' + (save_sequence[i]-120);
        }
        else if (Math.floor((save_sequence[i] -1) / 60) == 3){
            save_sequence[i] = 'D' + (save_sequence[i]-180);
        }
    }
       
    pinsOutput.value = save_sequence;
...
function subtractArrays(arr1, arr2) {
    for(var i=0; i<arr1.selection.data.length;i++){
...
function nextStep(){
    ...
    incrementalCurrentStep.textContent = "Текущая линия: " + (pointIndex + 1) + " |  Гвоздь " + save_sequence[pointIndex] + " к " + save_sequence[pointIndex + 1];
...
function onOpenCvReady() {
    ...
    setTimeout(function(){
        document.getElementById('status').innerHTML = 'Генератор готов.';
    }, 1000);
